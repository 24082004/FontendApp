...
const MyTicket = ({ route, navigation }) => {
  ...
  const initialTicketId = routeParams.ticketId || 
  ...
  const fetchTicketDetails = async (ticketId, showLoading = true) => {
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 404) {
          const fallbackData = {
            ...
            movieTitle: routeParams.movieTitle || routeParams.bookingData?.movieTitle,
            selectedSeats: routeParams.selectedSeats || routeParams.bookingData?.selectedSeats || [],
            selectedFoodItems: routeParams.selectedFoodItems || routeParams.bookingData?.selectedFoodItems || [],
            totalPrice: routeParams.totalPrice || routeParams.bookingData?.totalPrice || 0,
            seatTotalPrice: routeParams.seatTotalPrice || routeParams.bookingData?.seatTotalPrice || 0,
            foodTotalPrice: routeParams.foodTotalPrice || routeParams.bookingData?.foodTotalPrice || 0,
            ...
            paymentMethod: routeParams.paymentMethod || routeParams.bookingData?.paymentMethod || 'cash',
            status: routeParams.status || routeParams.bookingData?.status || 'pending_payment',
            createdAt: routeParams.createdAt || routeParams.bookingData?.createdAt || new Date().toISOString(),
            isOfflineData: true
          };
      ...
      if (error.message.includes('Network') || error.message.includes('fetch')) {
        ...
        setTicketData(fallbackData);
        setError('Đang hiển thị dữ liệu offline. Vui lòng kéo xuống để làm mới.');
        return fallbackData;
      }
  ...
  const validateTicket = async (ticketId) => {
    try {
      ...
      if (!response.ok) {
        throw new Error(responseData.message || responseData.error || 'Validation failed');
      }
  ...
  const getTicketData = (ticket) => {
    ...
    return {
      ...
      cinema: ticketData.cinema?.name ? ticketData.cinema : { name: ticketData.cinema },
      ...
      selectedFoodItems: ticketData.foodItems || ticketData.selectedFoodItems || [],
  ...
  const shareTicket = async () => {
    ...
    try {
      const message = `🎬 VÉ ĐIỆN TỬ CINEMA\n\n` +
        ...
        `💰 Tổng tiền: ${(ticketData.totalPrice || 0).toLocaleString('vi-VN')}đ\n` +
        `📝 Mã vé: ${ticketData.orderId || ticketData._id}\n` +
        `💳 Thanh toán: ${ticketData.paymentMethod === 'cash' ? 'Tại rạp' : 'Stripe'}\n` +
    ...
    <SafeAreaView style={styles.safeAreaHeader}>
      <View style={styles.header}>
        ...
        <View style={styles.headerActions}>
          ...
          {ticketData && (
            <TouchableOpacity onPress={shareTicket} style={styles.headerActionButton}>
  ...
  const renderEmptyState = () => (
    <View style={styles.container}>
      ...
      <View style={styles.emptyContainer}>
        ...
        >
          <Icon name="home-outline" size={16} color="#000" style={{ marginRight: 8 }} />
  ...
  const qrData = JSON.stringify({
    ticketId,
    movieTitle: extractObjectProperty(ticketData.movieTitle),
    cinema: extractObjectProperty(ticketData.cinema),
    room: extractObjectProperty(ticketData.room),
    showtime: formatShowtime(ticketData.showtime),
    seats: renderSeats(ticketData.selectedSeats),
    ...
    <View style={styles.container}>
      ...
      <View style={styles.content}>
        ...
        >
                ...
                <View style={styles.detailRow}>
                  <View style={styles.detailItem}>
                    ...
                    <View>
                      ...
                      <Text style={styles.detailValue}>
                        {Array.isArray(ticketData.selectedSeats) ? ticketData.selectedSeats.length : 0} vé
                      </Text>
                    </View>
                  </View>
                  <View style={styles.detailItem}>
                    ...
                    <View>
                      ...
                      <Text style={styles.detailValue}>
                        {ticketData.paymentMethod === 'cash' ? 'Tại rạp' : 'Online'}
                      </Text>
                    </View>
                  </View>
                </View>
          ...
          {/* Enhanced Food Items */}
          {ticketData.selectedFoodItems && ticketData.selectedFoodItems.length > 0 && (
            <View style={styles.infoCard}>
              <View style={styles.cardHeader}>
                ...
                {ticketData.selectedFoodItems.map((item, index) => {
                  ...
                  if (typeof item === 'object' && item !== null) {
                    ...
                    if (item.food && typeof item.food === 'object') {
                      itemName = item.food.name || item.food.title || `Item ${index + 1}`;
                    } else {
                      itemName = item.name || item.title || item.text || `Item ${index + 1}`;
                    }
                  ...
                  return (
                    <View key={index} style={styles.foodItem}>
                      <View style={styles.foodIcon}>
                        <Icon name="restaurant-outline" size={16} color="#FDC536" />
                      </View>
          ...
          <View style={styles.infoCard}>
            ...
            <View style={styles.notesList}>
              ...
              {(ticketData.status === 'pending_payment' || ticketData.status === 'pending') && (
                <View style={styles.noteItem}>
                ...
                Alert.alert(
                  ...
                  [
                    ...
                    { 
                      text: 'Thêm', 
                      onPress: () => {
                        Alert.alert('Thành công', 'Đã thêm lịch xem phim vào calendar!');
                      }
                    }
                  ]
                );
          ...
          <View style={styles.validationContainer}>
            <TouchableOpacity 
              ...
              onPress={async () => {
                const result = await validateTicket(ticketData._id || ticketData.orderId);
                if (result.valid) {
                  Alert.alert('Vé hợp lệ', 'Vé của bạn đã được xác thực thành công.');
                } else {
                  Alert.alert('Vé không hợp lệ', result.message || 'Không thể xác thực vé.');
                }
              }}
            >
          ...
          <View style={styles.footerLeft}>
            ...
            <Text style={styles.footerText}>
              {ticketData.createdAt 
                ? `${new Date(ticketData.createdAt).toLocaleDateString('vi-VN')} ${new Date(ticketData.createdAt).toLocaleTimeString('vi-VN', { hour: '2-digit', minute: '2-digit' })}`
